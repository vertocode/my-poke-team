...
<template class="sub-header">
  <nav class="navbar navbar-light row" style="background-color: #FFFFFF;">
    ...
    <div class="col-5 row p-1">
        <h2 class="text-end pt-1 col-6"><router-link to="/" class="link-nav" :class="{ 'link-active': routeActual === '/' }">{{ data.myTeams }}</router-link></h2>
        <h2 class="text-start pt-1 col-6"><router-link to="/createteam" class="link-nav" :class="{ 'link-active': routeActual === '/createteam' }">{{ data.createTeam }}</router-link></h2>
    </div>
  </nav>
</template>
